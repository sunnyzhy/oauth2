...
- 源码
   ...
   - token: new SqlLobValue(this.serializeAccessToken(token)), 把 OAuth2AccessToken 对象序列化为字节流，并封装成 SqlLobValue 类型的数据库对象
   - authentication_id: this.authenticationKeyGenerator.extractKey(authentication), 用 MD5 加密 username, client_id, scope 所生成的加密串
   - user_name: authentication.getName(), 上下文里的认证用户名
   - client_id: authentication.getOAuth2Request().getClientId(), 上下文里的client_id
   - authentication: new SqlLobValue(this.serializeAuthentication(authentication)), 把 OAuth2Authentication 对象序列化为字节流，并封装成 SqlLobValue 类型的数据库对象
   - refresh_token: this.extractTokenKey(refreshToken),  用 MD5 加密  refresh_token 所生成的 32 位加密串
